...
Um módulo para implementar uma rede neural com o aprendizado baseado no algoritmo Stochastic Gradient Descent como mini batches para uma rede neural feedforward. 
...
         O `mini_batch` é uma lista de tuplas `(x, y)`, e `eta` é a taxa de aprendizado."""
        ...
        #Atualiza todos os pesos utilizando a taxa de aprendizado eta
        #Aqui também divide a taxa de aprendizado pela quantidade de mini_batch
...
         a `self.biases` e `self.weights`."""
        ...
        #Passo Forward
        for b, w in zip(self.biases, self.weights):
            #Semelhante a função somatório vista no Perceptron X * W + b
        ...
        for l in range(2, self.num_layers):
            z = zs[-l]
            #Saída da derivada da função sigmoid
            sp = sigmoid_prime(z)
